...
%        <Item name="ObjectFinder Save filament as mask image" icon="Matlab" tooltip="Save filament as mask image.">
%          <Command>Matlab::ObjectFinderSaveFilamentAsMask(%i)</Command>
%        </Item>
%       </Submenu>
%      </Menu>
%      <SurpassTab>
...
function ObjectFinderSaveFilamentAsMask(aImarisApplicationID)
...
vAnswer = inputdlg({'Extra XY radius (um):\n',...
                    'Extra Z radius (um)',...
                    'Exclusion radius from cell soma (um, 0 for no exclusion)',...
                    'XY voxel size (um)', 'Z voxel size (um)'}, ...
                    'Choose Channel', 1, {'1','0.5','10', num2str(xyum), num2str(zum)});
...
SomaRad             = str2double(vAnswer(3));
vVoxSizeXY          = str2double(vAnswer(4));
vVoxSizeZ           = str2double(vAnswer(5));
