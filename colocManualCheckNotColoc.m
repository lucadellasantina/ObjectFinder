...
%
% Ask user to check only objects that were flagged as NOT colocalized
%
...
function Coloc = colocManualCheckNotColoc(Dots, Filter, Post, Colo, ColocManual, Colo2, ColocManual2)
if isempty(Colo2) % Single channel, check only colocalized objects
    % Find objects that are colocalized with both channels
    Coloc1 = find(ColocManual.ColocFlag==2); % Numbers of colocalized dots analyzed
    ...
    Coloc = colocVideoFig(ColocManual, Grouped, Post, Colo, Colo2, ColocManual2);
else % 2 Colocalizing channels, check only double-colocalized objects
    % Find objects that are colocalized with both channels
    Coloc1 = find(ColocManual.ColocFlag==2); % Numbers of colocalized dots analyzed
    Coloc2 = find(ColocManual2.ColocFlag==2); % Numbers of colocalized dots analyzed
